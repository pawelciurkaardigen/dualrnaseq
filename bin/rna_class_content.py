...
def get_gene_type_host(gene_RNAtype_counts, set_RNA_classes, gene_attribute):
    ...
    gene_RNAtype_counts_df.to_csv(
        "host_gene_types_groups_" + gene_attribute.split("_")[0] + ".tsv", sep="\t", index=False
    )
...
def RNA_classes_for_each_sample(
    quantification_table_path, gene_types, set_RNA_classes, organism, quantifier, gene_attribute
):
    ...
    for sample_name in quantification_table:
        ...
        if "NumReads" in sample_name:
            ...
            df_RNA_classes_sum_counts_all_samples = pd.concat(
                [df_RNA_classes_sum_counts_all_samples, df_RNA_classes_sum_counts.T], sort=True
            )
